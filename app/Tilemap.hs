...
import Raylib.Types (Vector2(..), Rectangle(..), Camera2D (..), Texture)
import Components (System', Tile(..), Chunk(..), Tilemap (..), CameraComponent (..))
import Foreign.C.Types (CFloat)
import Foreign.C
import GHC.Float (int2Float)
import Apecs
import System.Random (uniformR, initStdGen, newStdGen, randomRIO, randomIO)
import Utils (permutate, areBoxesColliding)
import Raylib (getScreenWidth, getScreenHeight, drawTextureRec)
...
checkVisibleTilemapChunks = do
  cmapM $ \(CameraComponent _ (Camera2D (Vector2 tx ty) _ _ zoom)) -> do
